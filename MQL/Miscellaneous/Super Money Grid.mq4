...
// EXTERN VARS
// ------------------------------------------------------------------------------------------------
extern int magic = 93793;
// Configuration
extern string www.lifesdream.org = "Super Money Grid v1.41";
extern string CommonSettings = "---------------------------------------------";
extern int user_slippage = 2; 
extern int grid_size = 60;
extern int take_profit = 65;
...
extern int progression = 2; // 0=none | 1:d'alembert | 2:martingale | 3:fibonacci
extern int unbalance_control = 1;

// ------------------------------------------------------------------------------------------------
// GLOBAL VARS
// ------------------------------------------------------------------------------------------------
string key = "Super Money Grid v1.41";
// Ticket
int buy_tickets[50];
int sell_tickets[50];
// Lots
...
void ShowData()
{
  string txt;
  double aux_tp_buy=0, aux_tp_sell=0;
  string info_money_management[3];
  ...
  txt = "\nCopyright © 2011, www.lifesdream.org" +
        "\nSuper Money Grid v1.41 is running." +
        "\n" +
        "\nSETTINGS: " + 
        "\nGrid size: " + grid_size +
...
void Robot()
{
  ...
  if (buys==1)
  {
    ...
    if (buy_profit[buys-1]<=CalculateSL(buy_lots[buys-1]))
    {
      ...
      if (progression==3) ticket = OrderSendReliable(Symbol(),OP_BUY,buy_lots[buys-1],MarketInfo(Symbol(),MODE_ASK),slippage,0,0,key,magic,0,Blue); 
    } 
  ...
  if (buys>1)
  {
    ...
    if (buy_profit[buys-1]<=CalculateSL(buy_lots[buys-1]))
    {
      ...
      if (buys<50)
      {
        ...
        if (progression==1) ticket = OrderSendReliable(Symbol(),OP_BUY,buy_lots[buys-1]+buy_lots[0],MarketInfo(Symbol(),MODE_ASK),slippage,0,0,key,magic,0,Blue); 
        if (progression==2) ticket = OrderSendReliable(Symbol(),OP_BUY,2*buy_lots[buys-1],MarketInfo(Symbol(),MODE_ASK),slippage,0,0,key,magic,0,Blue); 
        if (progression==3) ticket = OrderSendReliable(Symbol(),OP_BUY,buy_lots[buys-1]+buy_lots[buys-2],MarketInfo(Symbol(),MODE_ASK),slippage,0,0,key,magic,0,Blue); 
      }
  ...
  {
    // CASE 1 >>> We reach Stop Loss (grid size)      
    if (sell_profit[sells-1]<=CalculateSL(sell_lots[sells-1]))
    {
      // We are going to open a new order. Volume depends on chosen progression.
      ...
      if (progression==3) ticket = OrderSendReliable(Symbol(),OP_SELL,sell_lots[sells-1],MarketInfo(Symbol(),MODE_BID),slippage,0,0,key,magic,0,Red); 
    } 
    ...
    if (sell_max_profit>0 && sell_close_profit>0 && sell_max_profit>sell_close_profit && total_sell_profit<sell_close_profit) 
    {
      for (i=0; i<=sells-1; i++) 
      {
        closed=OrderCloseReliable(sell_tickets[i],sell_lots[i],MarketInfo(Symbol(),MODE_ASK),slippage,Blue);
      }
      // At this point all order are closed. Global vars will be updated thanks to UpdateVars() on next start() execution
      SellResetAfterClose(); 
    }    
  ...
  if (sells>1)
  {
    ...
    if (sell_profit[sells-1]<=CalculateSL(sell_lots[sells-1]))
    {
      ...
      if (sells<50)
      {
        ...
        if (progression==1) ticket = OrderSendReliable(Symbol(),OP_SELL,sell_lots[sells-1]+sell_lots[0],MarketInfo(Symbol(),MODE_BID),slippage,0,0,key,magic,0,Red); 
        if (progression==2) ticket = OrderSendReliable(Symbol(),OP_SELL,2*sell_lots[sells-1],MarketInfo(Symbol(),MODE_BID),slippage,0,0,key,magic,0,Red); 
        if (progression==3) ticket = OrderSendReliable(Symbol(),OP_SELL,sell_lots[sells-1]+sell_lots[sells-2],MarketInfo(Symbol(),MODE_BID),slippage,0,0,key,magic,0,Red); 
      }
